    ...
    <html lang="en">
      <head>
        <Script
          ...
          data-tenant={process.env.NEXT_PUBLIC_TRACKSOURCE_ID}
        />
      </head>
      <body>{children}</body>
    </html>
...
export function AuthHandler() {
  ...
  useEffect(() => {
    if (isLoaded && user && window.TracksSource) {
      window.TracksSource.identify(user.primaryEmailAddress?.emailAddress, {
        user_id: user.id,
        name: user.fullName,
...
export function TrackSourceIdentify() {
  ...
  useEffect(() => {
    if (session?.user && window.TracksSource) {
      window.TracksSource.identify(session.user.email, {
...
export function TrackSourceIdentify() {
  ...
  useEffect(() => {
    if (user && window.TracksSource) {
      window.TracksSource.identify(user.email, {
        user_id: user.id,
...
3. Set URL: `https://events.tracksource.io/payments/YOUR_TENANT_ID/stripe`
4. Select events:
...
async function handleCheckout(priceId: string) {
  const attribution = window.tsAttribution || {};
  const tsAnonymousId = attribution.anonymous_id;
...
TracksSource.track('trial_started', {
  plan: 'pro',
  trial_days: 14
});
```
...
TracksSource.track('feature_used', {
...
TracksSource.track('plan_upgrade', {
  from_plan: 'starter',
  to_plan: 'pro',
  conversion_value: 49
});
...
export function identifyUser(user: { email: string; id: string; name?: string }) {
  if (typeof window !== 'undefined' && window.TracksSource) {
    window.TracksSource.identify(user.email, {
      user_id: user.id,
      name: user.name
    });
...
export function trackEvent(event: string, properties?: Record<string, any>) {
  if (typeof window !== 'undefined' && window.TracksSource) {
    window.TracksSource.track(event, properties);
  }
}

export function getAnonymousId(): string | undefined {
  if (typeof window !== 'undefined' && window.tsAttribution) {
    return window.tsAttribution.anonymous_id;
  }
}
