...
Add to your GHL page's **Header Tracking Code** section:

```html
<!-- TrackSource SDK - Single line installation -->
...
<script>
  ...
  function updateTypeformWithTrackSource() {
    // Get TrackSource attribution data
    const attribution = window.tsAttribution || {};
    const tsId = attribution.anonymous_id || '';
...
<script>
  function populateGHLHiddenField() {
    ...
    if (attribution.anonymous_id) {
      ...
      if (hiddenField) {
        hiddenField.value = attribution.anonymous_id;
      }
    } else {
...
2. Create webhook:
   - URL: `https://events.tracksource.io/payments/YOUR_TENANT_ID/whop`
...
```html
<!-- TRACKSOURCE INTEGRATION - Add to GHL Header Tracking Code -->

<!-- 1. TrackSource SDK (single line) -->
<script src="https://events.tracksource.io/sdk/ts.min.js" data-tenant="YOUR_TENANT_ID"></script>

<!-- 2. Pass TrackSource ID to Typeform/Tally iframes (add to Body Tracking Code) -->
<script>
  (function() {
    let attempts = 0;
