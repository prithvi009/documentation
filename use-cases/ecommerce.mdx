    ...
    document.querySelectorAll('form[action="/cart/add"]').forEach(function(form) {
      form.addEventListener('submit', function(e) {
        if (window.ts) {
          ...
          window.ts.track('add_to_cart', productData);
        }
      });
    });
...
<script>
  ...
  document.addEventListener('click', function(e) {
    ...
    if (target.name === 'checkout' || target.closest('[name="checkout"]') ||
      ...
      if (window.ts) {
        window.ts.track('checkout_started', {
...
3. Configure:
   - Event: **Order payment**
   - URL: `https://events.tracksource.io/webhooks/shopify/YOUR_TENANT_ID`
   - Format: JSON
4. Save
...
function ts_track_purchase($order_id) {
    ...
    <script>
      if (window.ts) {
        ...
        window.ts.identify('<?php echo $order->get_billing_email(); ?>', {
...
3. Configure:
   ...
   - Status: Active
   - Topic: **Order created**
   - Delivery URL: `https://events.tracksource.io/webhooks/woocommerce/YOUR_TENANT_ID`
